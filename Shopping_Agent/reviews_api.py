# this will read the review from table reviews in store.db and return the aggregated review for a given product
...
def get_ratings_for_products(product_ids: list[int]) -> list[dict]:
    ...
    ratings_map = {r[0]: {"average_rating": round(r[1], 2), "review_count": r[2]} for r in result}
    return [
        {
            "product_id": pid,
